...
%     sl = 65 * ones(1, 21);
    ...
    smoothing_window = 1; % 7 for accurate evaluation
    TOTAL = smoothdata(TOTAL, 'gaussian', smoothing_window);
    TOTAL = TOTAL/(sum(TOTAL)*fixed_bin_width);
    
    common_bin_centers = common_bin_edges(1:end-1)+fixed_bin_width/2;
    
    L2 = compareToUniformDistribution(TOTAL,common_bin_centers);    
end
